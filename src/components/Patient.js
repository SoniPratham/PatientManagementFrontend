...
export default function Student(props) {
  const [patientId, setId] = useState(null);
  const [name, setName] = useState(null);
  const [address, setAddress] = useState(null);

  const { patientIds } = useParams(); // Get the Path Parameter from the URL
  const navigate = useNavigate();

  useEffect(() => {
    if (patientIds) {
      axios
        .get("http://localhost:8080/patientRecord/" + patientIds)
        .then((response) => {
  ...
  console.log("delete");
    event.preventDefault();
        ...
        axios.post("http://localhost:8080/patientRecord", patient)
        .then((response) => {
          if (response.data != null) {
  ...
  let updateStudent = (event) => {
    ...
    axios.put("http://localhost:8080/patientRecord/" + patientIds, patient).then((response) => {
      if (response.data != null) {
